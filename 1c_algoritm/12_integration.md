# 🔗 Инструкция: Настройка синхронизации 1С:УНФ ↔ 1С:Бухгалтерия
**ООО «КБМ» | Версия: 1.0 | Дата: 21.03.2026**

| **Ответственные** | Администратор 1С, Главный бухгалтер |
| :--- | :--- |
| **Цель** | Организация автоматического обмена данными между оперативным контуром (УНФ) и бухгалтерским контуром (БП). |
| **Режим обмена** | 🔄 **Односторонний:** Из УНФ в Бухгалтерию. Ввод первички в БП запрещен. |
...
1.  **1С:УНФ (Управление нашей фирмой):** Ведет весь оперативный учет (продажи, закупки, производство, склад, зарплата, CRM). Здесь работают менеджеры, снабженцы, мастера цехов.
...
### 🔑 Ключевые принципы
*   **Единая истина:** Первичные документы создаются **только в УНФ**.
*   **Автоматизация:** Данные (справочники и документы) автоматически передаются в БП через синхронизацию.
*   **Запрет дублирования:** В Бухгалтерии запрещено вручную создавать поступления, реализации или начисления зарплаты, если они есть в УНФ.
...
```mermaid
graph LR
    A[1С:УНФ] -->|Оперативный учет | B(Сотрудники)
...
*   **1С:УНФ:** ред. 3.0 (последний релиз).
...
2.  **В УНФ:** Проверьте, что у всех контрагентов заполнен **ИНН**. Это главный ключ для автоматического сопоставления.
3.  **Организации:** Убедитесь, что наименования и ИНН организации в обеих базах совпадают полностью.

---

## 4. ⚙️ Этап 2: Настройка в 1С:УНФ (Источник)
...
| :--- | :--- | :--- |
| **Направление** | Из этой программы (УНФ) | Мы отправляем данные в БП. |
...
Перед первым полноценным обменом необходимо告诉 системе, что «ООО Ромашка» в УНФ и «ООО Ромашка» в БП — это один и тот же клиент.
...
В 1С:УНФ нажмите кнопку **Синхронизировать**.
...
2.  Частые причины:
    ...
    *   «Не заполнен обязательный реквизит» → Найдите документ в УНФ и заполните поле (например, ставку НДС).
3.  Исправьте причину и нажмите **Синхронизировать** снова.

### 8.3. Перед закрытием месяца
1.  Убедитесь, что все документы за месяц ушли из УНФ в БП.
...
| **Ошибка «Нет прав доступа»** | Пользователь УНФ не имеет прав на чтение базы БП (при прямом подключении). | Дать права на файловую папку БП или проверить права пользователя 1С. |
...
- [ ] В УНФ у всех контрагентов заполнен ИНН.
- [ ] В БП удалены явные дубли контрагентов.
- [ ] В УНФ создана настройка синхронизации с БП.
