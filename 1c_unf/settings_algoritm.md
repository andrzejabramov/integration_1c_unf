# 📘 Инструкция: Первоначальная настройка прав доступа в 1С:УНФ
...
**Путь:** `Администрирование` → `Настройки системы` → `Вкладка "Функциональность"` (или кнопка `Больше возможностей`)
...
В 1С:УНФ права настраиваются через **Группы доступа**. Мы будем использовать готовые шаблоны системы, модифицируя их под нужды КБМ.
